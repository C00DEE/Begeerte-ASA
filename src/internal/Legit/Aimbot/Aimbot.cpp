...
namespace g_Aimbot {
	...
	void AutoSwapBrokenEquipment()
	{
		...
		bool bPrintHeartbeat = (globalTick % 300 == 0);

		// ==========================================
		// 建立一个映射表：WornArmor[部位ID] = 当前穿着的物品指针
		// ==========================================
		...
		for (int i = 0; i < 6; i++)
		{
			...
			int ticksSinceLastSwap = globalTick - lastSwapTick[typeIndex];
			if (ticksSinceLastSwap < 60) continue;
			...
			else
			{
				...
				currentDurability = CurrentEquippedItem->ItemDurability;
				if (currentDurability <= ALERT_DURABILITY)
				{
					bNeedsEquip = true;
				}
			}
			...
			if (bNeedsEquip)
			{
				...
				float bestDurability = -1.0f;
				SDK::FItemNetID* bestItemNetId = nullptr;
				...
				for (int j = 0; j < InventoryItems.Num(); j++)
				{
					...
					//  部位必须匹配
					if ((int)CandItem->MyEquipmentType != typeIndex) continue;
					...
					float candDur = CandItem->ItemDurability;//耐久最重要 优先高耐
					if (candDur > ALERT_DURABILITY && candDur > bestDurability)
					{
						bestDurability = candDur;
						bestItemNetId = &CandItem->ItemID;
						bFoundReplacement = true;
					}
				}

				if (bFoundReplacement && bestItemNetId)
				{

					SDK::FItemNetID netId = *bestItemNetId;
					PC->ServerEquipPawnItem(netId); 
